...
sudo cp "$script_dir/ops-aws-profile.sh" /usr/bin/ops-aws-profile

# Use select to prompt the user for a choice
PS3="Choose where to append 'source /usr/bin/ops-aws-profile' (enter number): "
options=(
...
do
    case $opt in
        "~/.bashrc (for bash users)")
            echo "source /usr/bin/ops-aws-profile" >> ~/.bashrc
            echo "Added to ~/.bashrc"
            break
            ;;
        "~/.zshrc (for zsh users)")
            echo "source /usr/bin/ops-aws-profile" >> ~/.zshrc
            echo "Added to ~/.zshrc"
            break
            ;;
        "~/.bash_profile (alternative for bash users)")
            echo "source /usr/bin/ops-aws-profile" >> ~/.bash_profile
            echo "Added to ~/.bash_profile"
            break
            ;;
        "~/.profile (general profile for bash)")
            echo "source /usr/bin/ops-aws-profile" >> ~/.profile
            echo "Added to ~/.profile"
            break
            ;;
        "~/.zprofile (for zsh users during login)")
            echo "source /usr/bin/ops-aws-profile" >> ~/.zprofile
